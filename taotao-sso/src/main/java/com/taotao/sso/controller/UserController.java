...
public class UserController {
	...
	public Object check(@PathVariable String param, @PathVariable Integer type, String callback){
		...
		if(StringUtils.isBlank(param)){
			result  = TaotaoResult.build(400, "验证内容不能为空");
		}
		if(type == null){
			result = TaotaoResult.build(400, "验证类型不能为空");
		}
		if(type != 1 && type != 2 && type != 3){
			result = TaotaoResult.build(400, "验证内容类型错误");
		}
		if(result != null){
			if(null != callback){
				//需要回调
	...
	@ResponseBody
	public TaotaoResult userLogout(@PathVariable("token") String token){
