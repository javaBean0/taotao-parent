...
public class UserServiceImpl implements UserService {
	...
	public TaotaoResult checkData(String content, Integer type) {
		...
		return new TaotaoResult().ok(false);
	}
	...
	public TaotaoResult userLogin(String username, String password, 
		...
		if(!DigestUtils.md5DigestAsHex(password.getBytes()).equals(user.getPassword())){
			return TaotaoResult.build(400, "用户名或者密码 不正确！");
		}
		//成成token
		String token = UUID.randomUUID().toString();
		//把用户信息存入redis种之前把密码清空
	...
	public TaotaoResult getUserByToken(String token){
		...
		if(StringUtils.isBlank(json)){
			return TaotaoResult.build(400, 	"登录信息已过期，请重新登录！");
		}
		//更新过期时间
		jedisClient.expire(REDIS_USER_SESSION_KEY + ":" + token, SSO_SESSION_EXPIRE);
	...
	public TaotaoResult userLogout(String token) {
		...
		return TaotaoResult.build(405, "您已经退出，无需再次退出!");
	}
