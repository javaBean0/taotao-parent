...
public class ItemServiceImpl implements ItemService {
	...
	@Value("${REDIS_INTM_EXPIRE}")
	private Integer REDIS_INTM_EXPIRE;
	...
	public TaotaoResult getItemBaseInfo(long itemId) {
		...
		try {
			...
			//设置key的有效期
			jedisClient.expire(REDIS_ITEM_KEY + ":" + itemId + ":base", REDIS_INTM_EXPIRE);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public TaotaoResult getItemDesc(Long itemId) {
		...
		try {
			...
			//设置key的有效期
			jedisClient.expire(REDIS_ITEM_KEY + ":" + itemId + ":desc", REDIS_INTM_EXPIRE);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public TaotaoResult getItemParam(Long itemId){
		...
		if(list != null && list.size() > 0){
			...
			try {
				...
				//设置key的有效期
				jedisClient.expire(REDIS_ITEM_KEY + ":" + itemId + ":param", REDIS_INTM_EXPIRE);
			} catch (Exception e) {
				e.printStackTrace();
			}
			return TaotaoResult.ok(tbItemParamItem);
